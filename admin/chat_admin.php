...
$id=$_GET['id'];
$sql=mysql_query("select * from chat where u_id=$id order by message_id ASC");
while ($s=mysql_fetch_array($sql)) {
    ...
    $curtime=$s['cur_time'];
    $sent=$s['sent'];
    $y=mysql_fetch_array(mysql_query("select * from student_details where u_id=$id"));
    $gender=$y['gender'];
    if($gender=="male"){$gender1='../male.png';}
    else{$gender1='../female.png';}
	$x=mysql_fetch_array(mysql_query("select * from user where u_id=$id"));
	$name=$x['uname'];
    if($sent=='n'){
    echo '
    <div class="item">
